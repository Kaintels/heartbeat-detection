...
class PerfEnv:
	def __init__(self, data, costs, ff):
		...
		self.x = data[data_val_idx, 0:-1].astype('float32')
		self.y = data[data_val_idx,   -1].astype('int32')
	...
	def step(self, action):
		for i in np.where(action >= CLASSES)[0]:
			self.mask[i, action[i] - CLASSES] = 1   # 선택한 것을 1로 바꿔줌
...
class Log:
	def __init__(self, data_val, costs, ff, brain):
		...
		for i in range(self.LEN):
			self.files.append( open("../saved/log/run_%d.dat" % i, mode) )

		self.perf_file = open("../saved/log/run_perf.dat", mode)
