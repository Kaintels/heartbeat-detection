...
def BOHB_optimization():
    ...
    parser.add_argument('--max_budget', type=float, help='Maximum number of epochs for training.', default=243)
    ...
    parser.add_argument('--backend',
                        ...
                        choices=['pytorch', 'keras'], default='pytorch')

    args = parser.parse_args()

    if args.backend == 'pytorch':
        from experiments.pytorch_worker import PyTorchWorker as worker
    else:
        from example_keras_worker import KerasWorker as worker
    ...
    # store results
    with open(os.path.join(args.shared_directory, 'results/result.pkl'), 'wb') as fh:
        pickle.dump(res, fh)
    ...
    for k, v in res.data.items():
        temp_config = v.config
    optimizer = temp_config['optimizer']
    lr = temp_config['lr']
    momentum = temp_config['momentum']
    activation = temp_config['activation']
    layers = temp_config['layers']

    return NS


if __name__ == "__main__":
    NS = BOHB_optimization()
    NS.shutdown()
